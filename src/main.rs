...
extern "C"
fn my_button_clicked(_fun: *mut c_void, data: *mut c_void, evt: *mut c_void) {
    ...
    let msg_dlg = MessageDialog::new(&parent, "Pushed!!", "The Button", OK);
    msg_dlg.showModal();
}

fn make_button<T: WindowMethods>(parent: &T) -> Button {
    let button = Button::new(parent, ID_ANY, "Push me!", 10, 10, 50, 30, 0);
    let closure = Closure::new(my_button_clicked as *mut c_void, parent.ptr());
    unsafe {
